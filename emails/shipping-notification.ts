...
export function shippingNotificationHtml(data: ShippingNotificationData): string {
  const { orderId, customerName, items, shippingAddress } = data
...
<head>
  ...
  <title>Your Order Has Shipped — Maison Élise</title>
</head>
          ...
          <tr>
            <td style="padding:40px 0 32px;text-align:center;">
              ...
              <p style="margin:0;font-size:13px;color:#8A8880;">
                ${customerName ? `${customerName.split(" ")[0]}, your` : "Your"} parcel is on its way.
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="text-align:center;">
              ...
              <p style="margin:0;font-size:11px;color:#8A8880;line-height:1.6;">
                Our concierge team will be in touch with tracking details shortly.<br>
                Questions? We are always here to assist.
              </p>
            </td>
          </tr>
...
export function shippingNotificationText(data: ShippingNotificationData): string {
  ...
  const itemLines = items
    .map(
      (i) =>
        `- ${i.name}${i.edition ? ` · ${i.edition}` : ""}${i.quantity > 1 ? ` ×${i.quantity}` : ""}`
    )
    .join("\n")

  return `MAISON ÉLISE — YOUR ORDER HAS SHIPPED
...
Our concierge team will be in touch with tracking details shortly.
